...
wardenRoomRouter.get("http://localhost:8000/api/v1/warden/room-details/", async (req, res) => {
  try {
...
wardenRoomRouter.post("http://localhost:8000/api/v1/warden/room-details/create", async (req, res) => {
  const { block } = req.user;
  try {
...
wardenRoomRouter.get("http://localhost:8000/api/v1/warden/room-details/getAllNotAssignedStudents", async (req, res) => {
  try {
    const { block } = req.user;
    if (!block)
      return res.status(400).json({ error: "Block not found", data: null });
...
wardenRoomRouter.get("http://localhost:8000/api/v1/warden/room-details/getAllNotAssignedRooms", async (req, res) => {
  try {
    const { block } = req.user;
    if (!block)
      return res.status(400).json({ error: "Block not found", data: null });
...
wardenRoomRouter.post("http://localhost:8000/api/v1/warden/room-details/assignRoom", async (req, res) => {
  try {
...
wardenRoomRouter.post("http://localhost:8000/api/v1/warden/room-details/removeStudent", async (req, res) => {
  try {
    const { studentId, roomId } = req.body;
    const { block } = req.user;
