...
facultyEventRouter.get("http://localhost:8000/api/v1/faculty/event/get-all-approved", async (req, res) => {
  try {
...
facultyEventRouter.post("http://localhost:8000/api/v1/faculty/event/approve-event", async (req, res) => {
  try {
...
facultyEventRouter.post("/http://localhost:8000/api/v1/faculty/event/reject-event", async (req, res) => {
  try {
    const { isHOD } = req.user;
    if (!isHOD) {
      return res.status(400).json({ message: "Not A HOD", data: null });
